...
def prepare_c_trials (c_trials,list_of_drug):
    c_trials['atccode']=c_trials['scientific_title'].str.upper().apply(lambda string : who_is_there(string,list_of_drug))
    c_trials = c_trials.apply(lambda x: x.astype(str).str.upper())
    return c_trials

def prepare_pubmed (pubmed,list_of_drug):
    pubmed['atccode']=pubmed['title'].str.upper().apply(lambda string : who_is_there(string,list_of_drug))
    pubmed = pubmed.apply(lambda x: x.astype(str).str.upper())
    return pubmed

def create_journal_ds(c_trials,pubmed):
    journal = pd.concat([pubmed[['journal','date','atccode']],c_trials[['journal','date','atccode']]])
    journal = journal.apply(lambda x: x.astype(str).str.upper())
    journal = journal.groupby('journal').agg(lambda column: ",".join(column)).reset_index('journal')
...
def main():
	...
	try:
		...
		print('The implementation of the journal table failed ! ')
	"""
	...
	try : 
		...
		drugs = drug.\
		merge(c_trials_f_drugs[['atccode','clinical_trials_id']], on='atccode', how='left').\
		merge(pubmed_f_drugs[['atccode','pubmed_id']], on='atccode', how='left').\
		merge(journal_f_drugs[['atccode','journal_id']], on='atccode', how='left').fillna("")
	except :
		print('the implementation of the drug table went wrong ! ')
	...
	try :
		...
		print('the write_preprocessed_data function broke')
